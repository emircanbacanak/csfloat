...
from app import CHEAPEST_ITEMS, get_listing_id, get_price, parse_price, safe_get_text, sanitize_filename, PURCHASED_ITEMS, save_purchased_item, load_purchased_items
        ...
        try:
            ...
            if buy_now_button:
                ...
                try:
                    ...
                    return False
                log_queue.put("Satın alma işlemi tamamlanıyor, 2 saniye bekleniyor...")
                return True
            else:
...
def search_items(queries, log_queue):
    ...
    for q in queries:
        ...
        try:
            ...
            if any(name in item['Name'] and condition in item['Condition'] for item in PURCHASED_ITEMS.values()):
                log_queue.put(f"{name} | {condition}: Zaten satın alınmış, atlanıyor.")
                continue
            active_queries.append((name, condition, parse_price(limit_str.strip())))
        except:
            log_queue.put(f"Hatalı format: {q}. Format 'Ürün Adı | Condition | Limit Fiyat' şeklinde olmalı.")
            continue
    
    if not active_queries:
        log_queue.put("Tüm ürünler zaten alındı!")
    ...
    for name, condition, limit_price in active_queries:
        try:
            log_queue.put(f"{name} | {condition}: Arama başlatılıyor...")
                            ...
                            if current_price <= limit_price:
                                ...
                                if clicked:
                                    ...
                                    if complete_purchase(driver, log_queue):
                                        ...
                                        save_purchased_item(purchased_data)
                                        log_queue.put(f"PURCHASED:{listing_id}")
